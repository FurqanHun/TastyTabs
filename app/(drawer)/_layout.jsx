    ...
    <View style={{ flex: 1, backgroundColor: "#fff" }}>
      ...
      <DrawerContentScrollView {...props} contentContainerStyle={{ paddingTop: 10 }}>
        {state.routes.map((route, index) => {
          const { drawerLabel, drawerIcon, drawerItemStyle } = descriptors[route.key].options;

        
          if (route.name === "settings" || drawerItemStyle?.display === "none") {
            return null;
          }
          ...
          return (
            ...
            >
              <View style={styles.itemContent}>
                {drawerIcon && drawerIcon({ color: focused ? "#fff" : "#333", size: 22 })}
                <Text style={[styles.itemLabel, { color: focused ? "#fff" : "#333" }]}>
                  {typeof drawerLabel === 'function' ? drawerLabel({color: 'red'}) : drawerLabel}
                </Text>
              </View>
            </TouchableOpacity>
          );
      ...
      <View style={styles.bottomSection}>
        ...
        >
          <View style={styles.itemContent}>
            ...
            />
            <Text style={[styles.itemLabel, { color: activeRouteName === "settings" ? "#fff" : "#333" }]}>
              Settings
            </Text>
          </View>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  drawerLogo: { width: 65, height: 65, borderRadius: 32.5, backgroundColor: "#fff" },
  drawerBrandName: { color: "#fff", fontSize: 22, fontWeight: "900", marginTop: 8 },
